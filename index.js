...
async function renderLines() {
    ...
    for (const company of Object.keys(highwayData.lines)) {
        for (const line of Object.keys(highwayData.lines[company])) {
            // Modify vertices
            for (const branch of Object.keys(highwayData.lines[company][line].vertices)) {
                const vertices = []
                for (const vertex of highwayData.lines[company][line].vertices[branch]) {
...
function showLine(companyName, lineName, line) {
    ...
    for (const station of line.stations) {
        ...
        stations.innerHTML = `${line.prefix}${highwayData.stations[station].lines[companyName][lineName]} ${highwayData.stations[station].name}<br>`
...
function showStation(station) {
    ...
    for (company in station.lines) {
        ...
        for (line in station.lines[company]) {
            ...
            code.innerHTML = `${highwayData.lines[company][line].prefix}${station.lines[company][line]} `
...
function listStation() {
    ...
    for (const station of highwayData.stations) {
        ...
        if (!stationFilters.type.includes(station.type == undefined ? 'station' : station.type)) continue
